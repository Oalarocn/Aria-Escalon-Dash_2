...
(function (root) {
  ...
  // ---- Medición para "una sola página continua" --------------------
  // Los PDF no tienen auto-alto, así que medimos la altura real del
  // contenido con un primer render (página altísima + sentinelas) y
  // luego generamos la página a la medida exacta.
  var PAGE = { width: 612, marginTop: 46, marginBottom: 52, maxHeight: 14000 };

  function buildMeasureDoc(records, opts) {
    opts = opts || {};
    var dd = buildDocDefinition(records, opts); // contenido normal
    dd.pageSize = { width: PAGE.width, height: 60000 }; // lienzo alto: todo en 1 página
    // Sentinelas invisibles al inicio y final del contenido
    dd.content.unshift({ text: '\u200b', id: 'ARIA_START', fontSize: 1, margin: [0, 0, 0, 0] });
    dd.content.push({ text: '\u200b', id: 'ARIA_END', fontSize: 1, margin: [0, 0, 0, 0] });

    var M = { firstTop: null, endTop: null };
    dd.pageBreakBefore = function (node) {
      if (node && node.startPosition) {
        var t = node.startPosition.top;
        if (M.firstTop === null || t < M.firstTop) M.firstTop = t;
        if (node.id === 'ARIA_END') M.endTop = t;
      }
      return false; // nunca forzamos saltos durante la medición
    };

    // Devuelve la altura de página que ajusta exactamente el contenido,
    // o null si la medición no fue posible o excede el máximo del PDF.
    dd.computePageHeight = function () {
      if (M.firstTop === null || M.endTop === null) return null;
      var h = Math.ceil(PAGE.marginTop + (M.endTop - M.firstTop) + PAGE.marginBottom + 3);
      if (h > PAGE.maxHeight) return null; // demasiado alto para una sola página
      return h;
    };
    return dd;
  }
  ...
  function buildDocDefinition(records, opts) {
    ...
    return {
      pageSize: opts.pageHeight ? { width: (opts.pageWidth || 612), height: opts.pageHeight } : 'LETTER',
  ...
  var AriaReport = {
    ...
    buildMeasureDoc: buildMeasureDoc,
    PAGE: PAGE
  };
